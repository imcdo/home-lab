{ config, pkgs, lib, ... }:
with lib;
let
  cfg = config.services.homelab.satisfactoryServer;
  christianSshPublicKey = "ssh-rsa  christian@grandlan.dev";
  ianSshPublicKey ="ssh-rsa  ian@DESKTOP-C07E16P";
  user = "satisfactory-server";
  group = "satisfactory-server";
  homeDir = "/home/${user}";
  satisfactoryDataDir = "${homeDir}/satisfactory";

in {
  options.services.homelab.satisfactoryServer = {
    enable = mkEnableOption "Enable Satisfactory Dedicated Server";

    sshKeys = mkOption {
      type = types.listOf types.str;
      default = [ christianSshPublicKey ianSshPublicKey ];
      description = "SSH public keys for users allowed to access the Satisfactory server";
    };

    memoryMax = mkOption {
      type = types.str;
      default = "12G";
      description = "Maximum memory the Satisfactory server can use (e.g., 8G, 1024M)";
    };

    cpuQuota = mkOption {
      type = types.str;
      default = "60%";
      description = "CPU quota for the Satisfactory server (e.g., 50%, 200%)";
    };

  };

  config = mkIf cfg.enable {
    networking.firewall.allowedTCPPorts = [
      7777
      8888
      8080
    ];
    networking.firewall.allowedUDPPorts = [
      7777
      15777
      15000
    ];

    users.groups."satisfactory-server" = {};
    users.users."satisfactory-server" =  {
      group = "satisfactory-server";
      isSystemUser = true;
      description = "Satisfactory Server User";
      shell = pkgs.bash;
      home = "/home/satisfactory-server";
      createHome = true;
      packages = with pkgs; [
        steamcmd
        git
        tmux
      ];
      openssh.authorizedKeys.keys = cfg.sshKeys;
    };

    # Satisfactory server

    systemd.services."satisfactory-server" = {
      description = "Satisfactory Dedicated Server";
      after = [ "network.target" ];
      wants = [ "network.target" ];
      wantedBy = [ "multi-user.target" ];
      requires = [ "network-online.target" ];

      serviceConfig = {
        User = user;
        Group = group;

        WorkingDirectory = satisfactoryDataDir;

        ExecStart = "${pkgs.steam-run}/bin/steam-run ${satisfactoryDataDir}/FactoryServer.sh -Port=7777 -BeaconPort=15777 -QueryPort=15000";

        # prevent systemd assuming it daemonizes itself
        Type = "simple";

        TimeoutStartSec = "5m";

        # Always restart on crash
        Restart = "always";
        RestartSec = "5s";

        MemoryAccounting = true;
        MemoryMax = cfg.memoryMax;

        CPUAccounting = true;
        CPUQuota = cfg.cpuQuota;
      };

      preStart = ''
        set -eu
        mkdir -p ${satisfactoryDataDir}
        chown ${user}:${group} ${satisfactoryDataDir}

        ${pkgs.steamcmd}/bin/steamcmd \
            +force_install_dir ${satisfactoryDataDir} \
            +login anonymous \
            +app_update 1690800 validate \
            +quit
      '';

    };
  };
}